	...
	: custom_dock_widget(tr("Game List"), parent), xgui_settings(guiSettings), xemu_settings(emuSettings)
{
	...
	auto append_action = [this](const QString& text)
	{
		m_columnActs.append(new QAction(text, this));
	};

	append_action(tr("Show Icons"));
	append_action(tr("Show Names"));
	append_action(tr("Show Serials"));
	append_action(tr("Show Firmwares"));
	append_action(tr("Show Versions"));
	append_action(tr("Show Categories"));
	append_action(tr("Show Paths"));
	append_action(tr("Show PlayStation Move"));
	append_action(tr("Show Supported Resolutions"));
	append_action(tr("Show Sound Formats"));
	append_action(tr("Show Parental Levels"));
	append_action(tr("Show Compatibilities"));
